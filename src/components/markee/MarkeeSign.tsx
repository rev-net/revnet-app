...
export function MarkeeSign() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoadError(null);
    try {
      const strategy = await fetchMarkeeSignData(REVNETS_STRATEGY);
      ...
      setData({
        ...
        minimumPrice:
          onchainMinPriceValue > BigInt(0)
            ? onchainMinPriceValue
  ...
  return (
    <>
      ...
      >
        {/* Sign body */}
        <div className="border border-zinc-200 rounded px-8 md:px-16 py-8 max-w-lg bg-white hover:border-zinc-400 transition-colors duration-200">
          <p className="font-mono text-zinc-700 text-lg group-hover:text-zinc-900 transition-colors duration-200 text-center leading-snug">
            {loading ? "loading..." : data.message}
          </p>
        </div>

        {/* Price badge — bottom center */}
        <span className="absolute -bottom-3 left-1/2 -translate-x-1/2 rounded-full border border-zinc-200 bg-white px-3 py-0.5 text-xs font-mono text-zinc-400 opacity-0 group-hover:opacity-100 group-hover:border-zinc-400 group-hover:text-zinc-600 max-sm:opacity-100 max-sm:border-zinc-400 max-sm:text-zinc-600 transition-all duration-200 whitespace-nowrap">
          {loading
            ? "···"
            : loadError
              ? "unavailable"
