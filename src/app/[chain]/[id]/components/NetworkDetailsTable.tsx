    ...
    <div>
      ...
      <div className="max-w-md space-y-4 p-2">
        ...
        <div className="space-y-1">
          ...
          <p className="text-sm text-black-300 italic">
            Note: The higher the tax, the less that can be accessed by cashing out at any given time, and the more that is left to share between remaining {formatTokenSymbol(token)} holders who cash out later. 
          </p>
          <p className="text-sm text-black-300 italic">
          ...
          <p className="text-sm text-black-300 italic">
            This works on a bonding curve with the formula `y = (ax/s) * ((1-r) + xr/s)` where: `r` is the cash out tax rate (from 0 to 1), `a` is the total amount in the revnet being accessed, `s` is the current token supply of {formatTokenSymbol(token)}, `x` is the amount of {formatTokenSymbol(token)} being cashed out.
          </p>
        </div>
